#include "tdisplays3promvsrboard_audio_codec.h"
...
static const char TAG[] = "Tdisplays3promvsrboardAudioCodec";

Tdisplays3promvsrboardAudioCodec::Tdisplays3promvsrboardAudioCodec(
    int input_sample_rate, int output_sample_rate, gpio_num_t mic_bclk,
    gpio_num_t mic_ws, gpio_num_t mic_data, gpio_num_t spkr_bclk,
    gpio_num_t spkr_lrclk, gpio_num_t spkr_data, bool input_reference) {
  duplex_ = true;
  ...
  CreateVoiceHardware(mic_bclk, mic_ws, mic_data, spkr_bclk, spkr_lrclk,
                      spkr_data);
  ...
  gpio_set_level(AUDIO_MIC_ENABLE, 0);
#endif
  ...
  ESP_LOGI(TAG, "Tdisplays3promvsrboardAudioCodec initialized");
}
...
void Tdisplays3promvsrboardAudioCodec::CreateVoiceHardware(
  ...
  i2s_pdm_rx_config_t mic_config = {
      .clk_cfg = I2S_PDM_RX_CLK_DEFAULT_CONFIG(
          static_cast<uint32_t>(input_sample_rate_)),
...
void Tdisplays3promvsrboardAudioCodec::SetOutputVolume(int volume) {
  volume_ = volume;
  AudioCodec::SetOutputVolume(volume);
}

void Tdisplays3promvsrboardAudioCodec::EnableInput(bool enable) {
#ifdef CONFIG_BOARD_TYPE_LILYGO_T_DISPLAY_S3_PRO_MVSRBOARD_V1_0
...
void Tdisplays3promvsrboardAudioCodec::EnableOutput(bool enable) {
  gpio_set_level(AUDIO_SPKR_ENABLE, enable);
  AudioCodec::EnableOutput(enable);
}

int Tdisplays3promvsrboardAudioCodec::Read(int16_t *dest, int samples) {
  if (input_enabled_) {
      size_t bytes_read;
...
int Tdisplays3promvsrboardAudioCodec::Write(const int16_t *data, int samples) {
  if (output_enabled_) {
    size_t bytes_read;
    ...
    i2s_channel_write(tx_handle_, output_data, samples * sizeof(int16_t),
