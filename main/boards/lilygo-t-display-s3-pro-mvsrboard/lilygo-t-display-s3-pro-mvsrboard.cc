...
#define TAG "LilygoTDisplays3ProMVSRBoardBoard"
...
class Cst2xxse : public I2cDevice {
 public:
  ...
  Cst2xxse(i2c_master_bus_handle_t i2c_bus, uint8_t addr)
  ...
  ~Cst2xxse() { delete[] read_buffer_; }

  void UpdateTouchPoint() {
    ReadRegs(0x00, read_buffer_, 6);
    tp_.num = read_buffer_[5] & 0x0F;
    tp_.x =
        (static_cast<int>(read_buffer_[1]) << 4) | (read_buffer_[3] & 0xF0);
    tp_.y =
        (static_cast<int>(read_buffer_[2]) << 4) | (read_buffer_[3] & 0x0F);
  }
...
class LilygoTDisplays3ProMVSRBoardBoard : public WifiBoard {
 private:
  ...
  void I2cDetect() {
    ...
    for (int i = 0; i < 128; i += 16) {
      ...
      for (int j = 0; j < 16; j++) {
        ...
        address = i + j;
        esp_err_t ret =
            i2c_master_probe(i2c_bus_, address, pdMS_TO_TICKS(200));
        if (ret == ESP_OK) {
          printf("%02x ", address);
  ...
  static void touchpad_daemon(void *param) {
    ...
    auto &board =
        (LilygoTDisplays3ProMVSRBoardBoard &)Board::GetInstance();
  ...
  void InitCst226se() {
    ...
    cst226se_ = new Cst2xxse(i2c_bus_, CST226SE_IIC_ADDRESS);
    xTaskCreate(touchpad_daemon, "tp", 4096, NULL, 5, NULL);
  }

  void InitSy6970() {
    ESP_LOGI(TAG, "Init Sy6970");
    sy6970_ = new Sy6970(i2c_bus_, SY6970_ADDRESS);
  }
  ...
  void InitSt7796Display() {
    ...
    ESP_ERROR_CHECK(
        esp_lcd_new_panel_st7796(panel_io, &panel_config, &panel));
  ...
  void InitRt9080() {
    gpio_config_t config = {};
    config.pin_bit_mask = BIT64(RT9080_EN);
    config.mode = GPIO_MODE_OUTPUT;
    config.pull_up_en = GPIO_PULLUP_DISABLE;
...
#endif
    gpio_config(&config);
    gpio_set_level((gpio_num_t)RT9080_EN, 1);
  }

 public:
  LilygoTDisplays3ProMVSRBoardBoard()
  ...
  virtual AudioCodec *GetAudioCodec() override {
    static Tdisplays3promvsrboardAudioCodec audio_codec(
        AUDIO_INPUT_SAMPLE_RATE, AUDIO_OUTPUT_SAMPLE_RATE,
        AUDIO_MIC_I2S_GPIO_BCLK, AUDIO_MIC_I2S_GPIO_WS,
        AUDIO_MIC_I2S_GPIO_DATA, AUDIO_SPKR_I2S_GPIO_BCLK,
        AUDIO_SPKR_I2S_GPIO_LRCLK, AUDIO_SPKR_I2S_GPIO_DATA,
  ...
  Cst2xxse *GetTouchpad() { return cst226se_; }
};

DECLARE_BOARD(LilygoTDisplays3ProMVSRBoardBoard);
